function [ellipse_out] = gauss2d_ellipse_byCI(mu, C, alpha, N)
% GAUSS2D_ELLIPSE_BYCI Compute a (1-ALPHA)% confidence interval ellipse from a 2d gaussian
%
%  ELLIPSE_OUT = GAUSS2D_ELLIPSE_BYCI(MU, C, ALPHA, [N])
%
...
%  axes at area that contains 1-ALPHA of the data error and points 
%  for an ellipse that circles the mean at the location of the (1-ALPHA)%
%  confidence interval.
%
...
%
%
%  w/ contributions by Shen Wang and Jason Osik
...
a = sqrt(chi2inv((1-alpha),2)).*sqrt((sin(phi)^4-cos(phi)^4)/(sin(phi)^2*C(1)*k-cos(phi)^2*C(4)*k));
b = sqrt(chi2inv((1-alpha),2)).*sqrt((sin(phi)^4-cos(phi)^4)/(sin(phi)^2*C(4)*k-cos(phi)^2*C(1)*k));
